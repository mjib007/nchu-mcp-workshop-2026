...
## 課程結構（總 160 min,含 10 min 休息;安排在 3 小時時段,餘為報到/Q&A/buffer）

| 段落 | 時間 | 主題 | 主要檔案 | 狀態 |
|------|------|------|---------|------|
| 第一段 | 20 min | **Why MCP** | `01-why-mcp.pptx`、`01-why-mcp-video.mp4`（3 min Manim 長片） | ✅ 初稿完成（17 張 + v2 影片）|
| 第二段 | 20 min | **How MCP Works** | `02-how-mcp-works.pptx`（22 張，含附錄）、`02-mcp-connection-video.mp4`（2:17 Manim 影片） | ✅ v3 完成（pptx 與影片已對齊）|
| 休息 | 10 min | | | |
| 第三段 | 30 min | **Agentic Tool Loop** | `03-agentic-tool-loop.pptx`（8 張，A2 cut）、`03-agentic-loop-video.mp4`（1:07 Manim 影片） | ✅ pptx + 影片 D 方案完成 |
| 第四段 | 70 min | **動手做（mini-project 實作）** | `04-hands-on-lab.pptx`（12 張）、`04-hands-on-lab.md` + `mini-project/` + `infra/` | ✅ 完成 |
...
- 10–25 min：L1 Step 1–2（觀察 + 換 JSON）
- 25–45 min：L1 Step 3–4（改 docstring + 重啟驗證）
- 45–60 min：交叉展示 + L2/L3 試玩 — 幾位老師用自己領域資料 demo
- 60–70 min：Q&A + 鋪陳 Segment 5
